...
object DistributedQueue {
  type ThrowableLike[X] = HasInstance[X => Throwable]
  type CanDump[X] = HasInstance[X => String]
}
...
private class DistributedQueueImpl[
  ...
  private val queue: DelayedQueue[C, L, X] = {
    DelayedQueue(getKey, callee, onDequeue)
  }
  ...
  private lazy val onDequeue: (X, Either[L, Y]) => Unit = (x, either) => synchronized {
    map remove x match {
      case Some(promise) => either match {
        case Left(e) =>
          promise failure implicitly[ThrowableLike[L]].instance(e)
        case Right(y) =>
          promise success y
      }
      case None =>
        val dumped = implicitly[CanDump[X]].instance(x)
